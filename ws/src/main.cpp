...
public:
    ...
    // вычислить архимедову силу по глубине
...
// Функция для создания 3D графика траектории
...
int main() {
    ...
    // Начальные условия
    FishTailSimulation::State initial_state(
        0.0,
        0.1,
        1.0,
        0.0,
    ...
    std::cout << "Starting fish motion simulation..." << std::endl;
    auto results = simulation.simulate(duration, dt, initial_state);
    
    std::cout << "\nSimulation completed! " << results.size() << " steps calculated.\n" << std::endl;
    
    // Вывод первых результатов
    // print_simulation_results(results);
    
    // Сохранение данных для построения графиков
    save_data_to_file(results, "trajectory_data.txt");
    ...
    if (!results.empty()) {
        ...
        std::cout << "Final position: X = " << last.x << ", Y = " << last.y << std::endl;
        std::cout << "Final velocity: Vx = " << last.vx << ", Vy = " << last.vy << std::endl;
        std::cout << "Final angle: θ = " << last.theta << " rad" << std::endl;
        std::cout << "Total distance: " << std::sqrt(last.x*last.x + last.y*last.y) << std::endl;
    }
